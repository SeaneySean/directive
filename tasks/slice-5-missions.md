...
Branch: `slice/5-missions`. Owner: Orpheus. Reviewer: the critic.
...
3. `src/game/strategy/missions.ts`: `availableMissions(state)`, `startMission(state, id)`
   (marks it in progress), `completeMission(state, id, result)` (grants `alien-artefact` or
   `aurichalcum`, applies the exposure reward, or on loss deducts 20 treasury and lets the
   mission be retried next turn). Missions are available from the turn they unlock.
...
5. Weaponry II upgrades squad weapons in missions (Plasma: range 7, accuracy 75, damage 5).
